...
def doCollectionCheckwithRecordsDIF(filename, titles, UMM='1.10', outputform = 'CSV', outfilename = 'result.csv'):
    ...
    #api = requests.get(filename)
    #parse the metadata file using the xml library
    xml = ElementTree.parse(api)
    #retrieve the structure of the xml file
    root_element = xml.getroot()
    ck = CollectionCheckerDIF.Checker(UMM)

    if (outputform == 'JSON'):
        for collection in root_element.iter('{http://gcmd.gsfc.nasa.gov/Aboutus/xml/dif/}'+'DIF'):
