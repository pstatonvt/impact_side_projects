...
class CollectionOutputJSON():
    ...
    def checkAll(self, metadata,titles,cid):
        ...
        str = 'Description'
        try:
            result[str] = self.checkerRules.checkDesc(metadata['Description'],titles['doi:' + metadata['DOI']['DOI']])
        except KeyError:
       ...
       # print metadata['Contacts']['Contact']
        ...
        try:
            ...
            xx = ""
            try:
                if metadata['Contacts']['Contact']['OrganizationPhones'] != None:
                    length = len(metadata['Contacts']['Contact']['OrganizationPhones'])
                    xx += self.checkerRules.checkContactPhone(metadata['Contacts']['Contact']['OrganizationPhones'],length) + ';'
                else:
                    xx += 'np'
            except:
                xx += 'np'
            result[str] = xx
        #Changed method for removing unnecessary 'np's FROM exceptions and also removed ';' from np
        #Changed by Siva 02/23/2018
        # ================
        ...
        str = 'OnlineAccessURLs/OnlineAccessURL/MimeType'
        try:
            # print metadata['OnlineAccessURLs']['OnlineAccessURL']
            result[str] = self.checkerRules.checkOnlineAccessURLMimeType(
                metadata['OnlineAccessURLs']['OnlineAccessURL']['MimeType'], 1)
        except TypeError:
            if metadata['OnlineAccessURLs'] != None:
                length = len(metadata['OnlineAccessURLs']['OnlineAccessURL'])
                result[str] = self.checkerRules.checkOnlineAccessURLMimeType(metadata['OnlineAccessURLs']['OnlineAccessURL'],
                                                                   length)
            else:
                result[str] = "np - reminder: a mime type should be provided for all SERVICE links (Service URL Types in UMM-Common: Access Map Viewer/ Access Mobile App/ Access Web Service/ DIF/ Map Service/ NOMADS/ Opendap Data/ OpenSearch/ SERF/ Software Package/ SSW/ Subsetter/ THREDDS Data)"
        except KeyError:
            result[str] = "np - reminder: a mime type should be provided for all SERVICE links (Service URL Types in UMM-Common: Access Map Viewer/ Access Mobile App/ Access Web Service/ DIF/ Map Service/ NOMADS/ Opendap Data/ OpenSearch/ SERF/ Software Package/ SSW/ Subsetter/ THREDDS Data)"
        except:
            result[str] = 'np'
        ...
        # ================
        #Added by Siva for checking Mime Type - 02/20/2018
        str = 'OnlineResources/OnlineResource/MimeType'
        try:
            result[str] = self.checkerRules.checkOnlineResourceMimeType(
                metadata['OnlineResources']['OnlineResource']['MimeType'], 1)
        except TypeError:
            try:
                if metadata['OnlineResources'] != None:
                    length = len(metadata['OnlineResources']['OnlineResource'])
